...
func RunServer(cfg lib.Config, requestHandler Handler) error {
	...
	router.HandleFunc("/api/v1/parking/health", requestHandler.HealthCheck).Methods(http.MethodGet)
	router.HandleFunc("/api/v1/parking/register", requestHandler.RegisterVehicle).Methods(http.MethodPost)
	router.HandleFunc("/api/v1/parking/exit", requestHandler.ExitVehicle).Methods(http.MethodPost)
